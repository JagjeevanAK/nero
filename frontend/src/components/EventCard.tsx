...
const EventCard: React.FC<EventCardProps> = ({
  ...
  >
    <div className="absolute left-0 top-0 z-20">
      <span
        className={`rounded-br-xl rounded-tl-2xl px-4 py-2 text-xs font-bold uppercase tracking-wider shadow-lg ring-2 ring-white/30 ${event.badgeColor} drop-shadow-md`}
      ...
      <p className="text-white/95 text-center mb-6 text-base font-medium leading-relaxed drop-shadow-sm">
        {event.description.split(" ").map((word, i) =>
          [
            ...
            "communication",
            "Linux",
          ].some((kw) => word.toLowerCase().includes(kw.toLowerCase())) ? (
            <span key={i} className="text-yellow-200 font-semibold ">
              {word}{" "}
            </span>
          ) : (
    ...
    <div className="flex flex-col mt-4 gap-5">
      <button
        className={`w-full font-bold rounded-lg py-2 text-base shadow-xl transition-all duration-300 border-0 outline-none bg-gradient-to-r ${
          ...
          .join(
            " "
          )} hover:text-white hover:shadow-2xl hover:scale-105 focus:ring-2 focus:ring-white/60`}
