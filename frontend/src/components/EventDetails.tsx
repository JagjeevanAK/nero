...
const EventDetailHeader: React.FC<Pick<EventDetailsProps, 'title' | 'icon' | 'accent'>> = ({ title, icon: Icon, accent }) => (
  <motion.div
...
const SectionCard: React.FC<Section> = ({ title, icon: Icon, accent = 'from-blue-900/40 to-indigo-900/20', children }) => (
...
const ContactCard: React.FC<{ coordinator: Coordinator }> = ({ coordinator }) => (
  <div className="rounded-2xl p-5 md:p-7 bg-gradient-to-br from-fuchsia-900/30 to-blue-900/20 border border-fuchsia-400/10 shadow-xl flex flex-col gap-2 items-start mt-4">
    ...
    <div className="ml-2">
      <div className="mb-1"><span className="font-semibold text-blue-200">Event Coordinator:</span> {coordinator.name}</div>
      <div className="mb-1"><span className="font-semibold text-blue-200">Contact No:</span> {coordinator.phone}</div>
      <div><span className="font-semibold text-blue-200">Email:</span> <a href={`mailto:${coordinator.email}`} className="underline text-fuchsia-300 hover:text-fuchsia-200">{coordinator.email}</a></div>
    </div>
  </div>
);

const EventDetails: React.FC<EventDetailsProps> = ({ title, icon, accent, details, sections, coordinator }) => {
    ...
    <div className="min-h-screen theme-bg theme-text py-8 px-2 md:px-10 flex items-center justify-center">
      <div className="w-full max-w-3xl glass-card rounded-2xl flex flex-col gap-6 p-6 md:p-12">
        ...
        {sections.map((sec, idx) => (
          <SectionCard key={idx} title={sec.title} icon={sec.icon} accent={sec.accent}>
            {sec.children}
          </SectionCard>
        ))}
